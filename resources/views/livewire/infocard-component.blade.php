...
@endphp
    ...
    <div class="bg-white rounded-md shadow-md overflow-hidden max-w-4xl mx-auto mb-6">
        <div class="w-full d2c p-4 text-white">
            <h1>Infocards</h1>
        </div>
        <div class="p-4">
        ...
        <div class="flex flex-row flex-wrap">
            <div class="mx-2">
                ...
                <input wire:model="title" id="title" type="text" class="form-input">
            </div>
            <div class="mx-2">
                <label for="text" class="block form-label">Información</label>
                <textarea wire:model="text"  name="text" id="text" cols="30" rows="5" class="form-textarea"></textarea>
            </div>
            <div class="mx-2">
                <label for="type" class="block form-label">Color Énfasis</label>
        ...
        @if ($formAction=="store")
            <div class="w-full d2c p-4">
                <button wire:click="store" class="bg-green-500 hover:bg-green-700 text-white font-bold px-4 py-2 rounded text-xs">Crear</button>
            </div>
        @else
            <div class="w-full d2c p-4">
                <button wire:click="update" class="bg-blue-500 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded text-xs">Actualizar</button>
                <button wire:click="default" class="bg-yellow-400 hover:bg-yellow-500 text-white font-bold px-4 py-2 rounded text-xs">Cancelar</button>
            </div>
        @endif
            ...
            <div class="flex flex-wrap -mx-1 lg:-mx-4">
                ...
                <div class="my-1 px-1 w-full md:w-1/2 lg:my-4 lg:px-4 lg:w-1/3">
                    ...
                    <article class="overflow-hidden rounded-lg shadow-lg">
                        ...
                        <header class="items-center justify-between p-2 md:p-4">
                            <p class="text-xs">{{$infocard->updated_at->format('d-m-Y')}}</p><hr>
                            <pre class="text-gray-800 font-sans w-max-content bg-yellow-100">
                                {{$infocard->text}}
                            </pre>
                        </header>
                        ...
                        <footer class="flex items-center justify-between leading-none p-2 md:p-4">
                            <a class="flex items-center no-underline hover:underline text-black" href="#">
                                <img class="h-8 w-8 rounded-full object-cover" src="{{ $infocard->user->profile_photo_url }}" alt="{{ $infocard->user->name }}" />
                                <p class="ml-2 text-sm">
                                    {{$infocard->user->name}}
                                </p>
                            </a>
                            <a class="no-underline text-grey-darker hover:text-red-dark" href="#">
                                <button wire:click="edit({{$infocard}})" class="bg-blue-500 hover:bg-blue-700 text-white font-bold px-4 py-2 rounded text-xs">Editar</button>
                                <button wire:click="destroy({{$infocard}})" class="bg-red-500 hover:bg-red-700 text-white font-bold px-4 py-2 rounded text-xs">Borrar</button>
                            </a>
                        </footer>
