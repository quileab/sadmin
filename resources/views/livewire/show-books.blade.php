<div wire:init="loadData">
  ...
  <div class="w-full flex items-center d2c px-4">
      ...
      <tr>
        <th scope="col" class="cursor-pointer px-4 py-3 text-left text-xs font-medium uppercase tracking-wider"
          ...
          @if ($sort != 'id')
            <x-sortNone />
          @else
            @if ($direction == 'asc') <x-sortUp /> @else <x-sortDown /> @endif
          @endif
        </th>
        <th scope="col" class="cursor-pointer px-4 py-3 text-left text-xs font-medium uppercase tracking-wider"
    ...
    <tbody class="bg-white divide-y divide-gray-200">
      @foreach ($books as $book)
        <tr class="hover:bg-gray-100">
          ...
          <td class="px-6 py-4">
            <div class="text-sm font-semibold text-gray-900" title="{{ $book->synopsis}}">{{ $book->title }}</div>
          </td>
          <td class="px-6 py-4">
          ...
          <td class="px-6 py-4">
            @if ($book->user)
              <span
                class="px-2 text-center block text-xs leading-5 font-semibold rounded-full bg-red-200 text-red-900">
              ...
              @if ($book->user_id == 0)
                @if (session()->has('bookmark'))
                  <button wire:click="bookLendReturn('{{ $book->id }}',true)">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" stroke="green" fill="currentColor"
